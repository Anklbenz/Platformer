...
{
    [CreateAssetMenu]
    public sealed class Move : ScriptableObject, IMove
    {
        [Header("JumpSettings")]
        ...
        public  CapsuleCollider InteractCollider {get => _collider;}
        ...
        private Vector3 _groundCheckPlatformSize { get => new Vector3(_collider.bounds.size.x, _boxCastWidth, _collider.bounds.size.z * BOX_INDENT * transform.localScale.z); }
        private Vector3 _wallContactPlatformSize { get => new Vector3(_collider.bounds.size.x, _collider.bounds.size.y * BOX_INDENT * transform.localScale.y, _boxCastWidth); }

        private float _groundCheckDistance { get => (_collider.bounds.size.y / 2) * transform.localScale.y; }
        private float _wallCheckDistance { get => (_collider.bounds.size.z / 2) * transform.localScale.z; }

        
        public bool IsGrounded {
            get => Physics.BoxCast(_collider.bounds.center, _groundCheckPlatformSize / 2, Vector3.down, out RaycastHit groundHit,
                transform.rotation, _groundCheckDistance, _isGroundedLayer);
        }
        private bool isWallContact {
            get => Physics.BoxCast(_collider.bounds.center, _wallContactPlatformSize / 2, _moveDirection, out RaycastHit wallHit,
        ...
        private void OnDrawGizmos() {
            ...
            Gizmos.DrawLine(_collider.bounds.center, _collider.bounds.center - transform.up * _groundCheckDistance);
            Gizmos.DrawWireCube(_collider.bounds.center - transform.up * _groundCheckDistance, _groundCheckPlatformSize);

            Gizmos.color = Color.magenta;
            Gizmos.DrawWireCube(_collider.bounds.center + _moveDirection * _wallCheckDistance, _wallContactPlatformSize);
        }
    }
}
*/
